...
public class Substring {
     ...
     * (~3-5 ms против ~7-9 ms). На больших текстах эта разница будет сильно расти. Но реализация через {@code String.hashCode()} рано или поздно даст ошибку,
...
//        BigInteger prime = BigInteger.probablePrime(new Random().nextInt(16 - 3) + 3, new Random()); // Random number with length in range [3; 16] bit
    ...
    /**
     * <p>
     * Нахождение подстроки c помощью конечного автомата.
     * </p>
     * <p>
     * Алгоритм использует таблицу состояний конечного автомата, составленную на основе паттерна.
     * </p>
     * @param string Строка
     * @param pattern Искомая подстрока
     * @return Массив с индексами начала подстрок в строке или пустой массив, если строка не содержит искомой подстроки.
    ...
    /**
     * Функция расчета следующего состояния на основе строки, предыдущего состояния и текущего символа.
     * @param str Строк
     * @param state Предыдущее состояние
     * @param ch Символ
     * @return Следующие состояние
     */
    private static int getNextState(String str, int state, char ch) {
        ...
        // Не удалось вернутся на состяние в отрезке [1:state] поэтому необходимо вернутся в начало автомата
        return 0;
    }
